...
perm.quint <- quantile(ca.df$mean, probs =c(0,0.25,0.5,0.75,1))
perm.filt <- ca.df %>%
  filter(., mean>perm.quint[[3]])

colnames(ca.df)

wvc.drop <- ca.df %>% # drop the zeros
  filter(., Wld_m_y>0)
...
only.aadt <- ca.df %>%
  filter(AADT>=15000)
st_write(only.aadt, "data/processed/ca/visuals/ca_aadt_only.shp")

high.aadt <- map1.dat %>%
  filter(AADT>=15000)
st_write(high.aadt, "data/processed/ca/visuals/ca_highaadt_wvc_perm.shp")

high.aadt.50perm <- ca.df %>%
  filter(., mean>perm.quint[[3]]) %>%
  filter(AADT>=15000)
